...
DROP PROCEDURE IF EXISTS InsertarFotosUsuarios;  -- PUNTO 13 DEL MANUAL SP que inserta N registros de una tabla temporal
delimiter //
...
BEGIN
    ...
    SET validacion=1;
    CREATE TEMPORARY TABLE URLGenerados( -- CREACIÓN TABLA TEMPORAL
    ...
    BEGIN
		...
		DECLARE RecorrerURLSGenerados CURSOR FOR 		-- CURSOR 
        ...
        ciclo: LOOP
			...
			INSERT INTO Fotos(URL, latitud_foto, longitud_foto, deleted, Fecha, userid) -- INSERCIÓN
			VALUES(URL_aux, lon, lat, 0, CURDATE(), userid_aux);
       END LOOP;
       CLOSE RecorrerURLSGenerados;
       DROP TABLE URLGenerados; -- DROP TABLA TEMPORAL
	END;
	COMMIT;
